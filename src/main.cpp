...
#if defined(NDEBUG)
# error "bitnitron cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2810f56f1ab4ffaa856d3892e82d5702532e1367f40356eb521d4b991ed90426");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // bitnitron: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "bitnitron Signed Message:\n";
...
{
    // bitnitron: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // bitnitron: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // bitnitron: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1522680929, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Bitnitron is started in APRIL 2018 AND is based on Scrypt Algorithm";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047444de0e64f35044348e52394b5f7755272163aa289a79fc416d6aef7548fb1f6e04e5aa734caa12388b01396b1ba2b2804a8cc260f6fa65c7723a138c355095") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522680929;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1230795;
        ...
        assert(block.hashMerkleRoot == uint256("0x2d642aa5d11fc6f0a09051997c544cb8ce126821349f7df9262df6dddcb62c2c"));
...
unsigned char pchMessageStart[4] = { 0xe9, 0x83, 0x97, 0xf1 }; // bitnitron: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("bitnitronMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("bitnitronMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static bitnitronMiner(CWallet *pwallet)
{
    printf("bitnitronMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitnitron-miner");
    ...
    try { loop {
        ...
        printf("Running bitnitronMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("bitnitronMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&bitnitronMiner, pwallet));
}
